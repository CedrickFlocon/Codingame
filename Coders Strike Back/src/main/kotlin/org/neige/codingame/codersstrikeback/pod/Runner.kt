...
class Runner(game: Game, coordinate: Coordinate, speed: Scalar, angle: Int, nextCheckpoint: Checkpoint) : Ally(game, coordinate, speed, angle, nextCheckpoint) {
    ...
    override fun move(): Move {
        return if ((game.opponents + game.pods.find { it != this }!!).any({ willCollide(it) && (speed - it.speed).magnitude() > 300 })) {
            ...
            if ((1..4).any({ nextCheckpoint.isInCheckpoint(coordinate + (speed * it.toDouble())) && speed.magnitude() > it * 200 })) {
