    ...
    override fun move(): Move {
        return if (game.opponents.any({ willCollide(it) && (speed - it.speed).magnitude() > 200 })) {
            ...
            if (opponent.coordinate.distanceFrom(opponent.nextCheckpoint.coordinate) < coordinate.distanceFrom(opponent.nextCheckpoint.coordinate)) {
                ...
                when {
                    coordinate.distanceFrom(opponentCheckpoint.coordinate) > 5000 -> Move(game.nextCheckpoint(opponent.nextCheckpoint).coordinate, "200")
                    coordinate.distanceFrom(opponentCheckpoint.coordinate) > 2500 -> Move(game.nextCheckpoint(opponent.nextCheckpoint).coordinate, "100")
